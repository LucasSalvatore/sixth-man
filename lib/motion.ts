...
/**
 * Interpolates from the value it last held to `target`. Used for the gainWins
 * headline on team change — it moves between two teams' figures rather than
 * recounting from zero, which is reserved for first arrival in the hero.
 */
...
/** Fires `true` once, on the next frame after mount — for first-paint entrances. */
...
/** Stable callback identity for event handlers passed into effects. */
